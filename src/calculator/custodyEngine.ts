...
export function calculateWechselmodell(input: CalculationInput): CalculationResult {
  ...
  for (let i = 0; i < input.children.length; i++) {
    ...
    const capCheckDescA = isBuergergeldA
      ...
      : isCappedA
        ? `• ${input.parentA.name || "Elternteil A"}: Fiktiver Tabellenbedarf bei Alleinhaftung (${incA.adjustedNet.toFixed(2)} €, Gruppe ${appliedDtTierA.tierIndex}): ${individualTableRateA.toFixed(2)} € / Monat (Barunterhaltspflicht ungekappt ${rawChildObligationA.toFixed(2)} € > ${individualTableRateA.toFixed(2)} € -> auf Einzeleinkommensbedarf gedeckelt: ${childObligationA.toFixed(2)} €)`
        : `• ${input.parentA.name || "Elternteil A"}: Fiktiver Tabellenbedarf bei Alleinhaftung (${incA.adjustedNet.toFixed(2)} €, Gruppe ${appliedDtTierA.tierIndex}): ${individualTableRateA.toFixed(2)} € / Monat (errechnete Barunterhaltspflicht ${childObligationA.toFixed(2)} € liegt sicher unterhalb des Einzeleinkommensbedarfs)`;

    const capCheckDescB = isBuergergeldB
      ? `• ${input.parentB.name || "Elternteil B"}: Bürgergeld-Bezug (Barunterhaltspflicht = 0,00 €)`
      : isCappedB
        ? `• ${input.parentB.name || "Elternteil B"}: Fiktiver Tabellenbedarf bei Alleinhaftung (${incB.adjustedNet.toFixed(2)} €, Gruppe ${appliedDtTierB.tierIndex}): ${individualTableRateB.toFixed(2)} € / Monat (Barunterhaltspflicht ungekappt ${rawChildObligationB.toFixed(2)} € > ${individualTableRateB.toFixed(2)} € -> auf Einzeleinkommensbedarf gedeckelt: ${childObligationB.toFixed(2)} €)`
        : `• ${input.parentB.name || "Elternteil B"}: Fiktiver Tabellenbedarf bei Alleinhaftung (${incB.adjustedNet.toFixed(2)} €, Gruppe ${appliedDtTierB.tierIndex}): ${individualTableRateB.toFixed(2)} € / Monat (errechnete Barunterhaltspflicht ${childObligationB.toFixed(2)} € liegt sicher unterhalb des Einzeleinkommensbedarfs)`;
