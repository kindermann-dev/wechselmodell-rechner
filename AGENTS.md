...
`wechselmodell-rechner` is a high-precision, deterministic TypeScript calculation engine and React single-page application for computing child support in a **symmetrical 50:50 alternating custody model** (*paritätisches Wechselmodell*) under German family law.
...
- **Occupational Expenses (*Berufsbedingte Aufwendungen*)**:
  ...
  - If unemployed: $0\ €$.
- **Private Retirement Provision (*Zusätzliche Altersvorsorge*)**:
...
2. **Liability Incomes & Quotas ($Q_A, Q_B$)**:
   ...
   - *Mangelfall fallback*: If $H_{\text{total}} = 0$, evaluate against necessary retention ($SB_{\text{notw}} = 1.450\ €$ employed / $1.200\ €$ unemployed).
3. **Real Housing Additional Need (*Wohnmehrbedarf*, BGH XII ZB 565/15 Rn. 25)**:
...
4. **In-Kind Care Deduction (*Naturalunterhalt*, 50%)**:
...
7. **Net Settlement (*Spitzabrechnung*, $Z_A$)**:
